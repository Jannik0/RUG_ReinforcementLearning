...
class Agent:
    def __init__(self, q_net, v_net, target_net, memory, batch_size, discount_factor, actionspace_size):
        self.q_net = q_net
        self.v_net = v_net
    ...
    def train(self):
        ...
        for i in range(self.batch_size):
            ...
            SOFTMAX_CURRENT[i] = np.exp(v_targets[i]) / (self.actionspace_size * np.exp(v_targets[i]))
...
def main():
    ...
    path = './Data/Div-DQV/' + environment_id + '/' + session_id + '/'
    if not os.path.exists(path):
        os.makedirs(path)
    print("This is Div-DQV " + environment_id + " " + session_id + "\nSession will be stored at " + path)
    ...
    if os.path.isfile(path + 'agent.pkl'):
        ...
        target_net = Network('v', actionspace_size, learning_rate, gradient_momentum, gradient_min).model
        memory = deque(maxlen=1000000)
        agent = Agent(q_net, v_net, target_net, memory, batch_size, discount_factor, actionspace_size)
        step_number = 0
